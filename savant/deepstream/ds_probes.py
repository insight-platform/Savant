...
def move_frames_to_batch_pad_probe(
    ...
    logger.debug(
        'Moving frames to batch to %s in buffer with PTS %s.',
        stage,
        buffer.pts,
    )
    frames_ids = []
    ...
    if not frames_ids:
        logger.debug(
            'Moving frames to batch to %s in buffer with PTS %s. Batch is empty.',
            stage,
            buffer.pts,
        )
    ...
    logger.debug(
        'Moving frames to batch to %s in buffer with PTS %s. Frames IDs: %s.',
        stage,
        buffer.pts,
        frames_ids,
    )
...
def move_batch_as_is_pad_probe(
    ...
    buffer: Gst.Buffer = info.get_buffer()
    logger.debug('Moving batch to %s in buffer with PTS %s.', stage, buffer.pts)
    nvds_batch_meta = pyds.gst_buffer_get_nvds_batch_meta(hash(buffer))
    try:
        next(nvds_frame_meta_iterator(nvds_batch_meta))
    except StopIteration:
        logger.debug(
            'Moving batch to %s in buffer with PTS %s. Batch is empty.',
            stage,
            buffer.pts,
        )
        return Gst.PadProbeReturn.OK
...
def move_batch_to_frames_pad_probe(
    ...
    buffer: Gst.Buffer = info.get_buffer()
    logger.debug(
        'Moving batch to frames to %s in buffer with PTS %s.',
        stage,
        buffer.pts,
    )
    nvds_batch_meta = pyds.gst_buffer_get_nvds_batch_meta(hash(buffer))
    try:
        ...
        logger.debug(
            'Moving batch to frames to %s in buffer with PTS %s. Batch is empty.',
            stage,
            buffer.pts,
        )
        return Gst.PadProbeReturn.OK
    ...
    batch_id = savant_frame_meta.idx if savant_frame_meta else None
    logger.debug(
        'Moving batch to frames to %s in buffer with PTS %s. Batch ID: %s.',
        stage,
        buffer.pts,
        batch_id,
    )
