...
class NvDsDrawFunc(BaseNvDsDrawFunc):
    ...
    def _get_obj_draw_spec(self, config:dict) -> ObjectDraw:
        ...
        if 'bbox' in config:    
            ...
            bbox_draw = BoundingBoxDraw(
                color=ColorDraw(*self._convert_hex_to_rgba(config['bbox']['color'])),
        ...
        if 'central_dot' in config:
            central_dot_draw = DotDraw(
                color=ColorDraw(*self._convert_hex_to_rgba(config['central_dot']['color'])),
        ...
        if 'label' in config:
            label_draw = LabelDraw(
                color = ColorDraw(*self._convert_hex_to_rgba(config['label']['color'])),
                font_scale=2.5,
                thickness=2,
    ...
    def _convert_hex_to_rgba(self, hex_color: str) -> Tuple[int, int, int, int]:
