...
class CpuImage:
    ...
    def cut(self, bbox: Union[BBox, RBBox]) -> Tuple['CpuImage', Union[BBox, RBBox]]:
        ...
        if isinstance(bbox, BBox):
            ...
            assert cut_left >= 0 and cut_right < self.width \
                and cut_top >= 0 and cut_bottom < self.height, \
            ...
            if start_row < 0 or start_col < 0 or end_col >= self.width \
    ...
    def rotate(
        ...
        rotation_matrix, resolution = \
            get_rotation_matrix(self, angle, (bbox.x_center, bbox.y_center))
...
class GpuImage:
    ...
    def cut(self, bbox: Union[BBox, RBBox], ) -> Tuple['GpuImage', Union[BBox, RBBox]]:
        ...
        if isinstance(bbox, BBox):
            ...
            assert start_col >= 0 and end_col < self.width \
                and start_row >= 0 and end_row < self.height, \
                f"bbox (left={bbox.left}, right={bbox.right}, top={bbox.top}, " \
                f"bottom={bbox.bottom}) is out of " \
                f"image size {self.width}x{self.height}"
            ...
            if start_row < 0 or start_col < 0 or end_col >= self.width \
                ...
                cut_roi = self._gpu_image \
                    .colRange(
                        max(start_col, 0),
                        min(end_col, self.width-1))\
                    .rowRange(
                        max(start_row, 0),
                        min(end_row, self.height-1))
                res_start_col = max(-start_col, 0)
                res_start_row = max(-start_row, 0)
                roi_res_image = res_image\
    ...
    def concat(self, image: 'GpuImage', axis: int = 0) -> 'GpuImage':
        ...
        if axis == 0:
            ...
            self.gpu_mat.copyTo(
                stream=self._cuda_stream,
                dst=res_image.rowRange(0, self.height)
            )
            ...
            self.gpu_mat.copyTo(
                stream=self._cuda_stream,
                dst=res_image.colRange(0, self.width)
            )
            image.gpu_mat.copyTo(
                stream=self._cuda_stream,
    ...
    def resize(
        ...
        start_col = (resolution[0] - new_resolution[0]) // 2
        start_row = (resolution[1] - new_resolution[1]) // 2
        res_roi = res.colRange(start_col, start_col + new_resolution[0])\
            .rowRange(start_row, start_row + new_resolution[1])
