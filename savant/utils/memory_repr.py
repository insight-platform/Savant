...
class OpenCVGpuMatWrapper:
    def __init__(self, gpu_mat: cv2.cuda.GpuMat):
        ...
        self.__cuda_array_interface__ = {
            ...
            if channels > 1 else (gpu_mat.step, gpu_mat.elemSize()),
        }
...
def as_opencv(
    ...
    if torch_imported and isinstance(img, torch.Tensor):
        ...
        if img.dim() == 3 and (img.shape[channel_index] > 4 or img.shape[channel_index] == 2):
            raise ValueError(
        ...
        if device.type == 'cuda':
            ...
            if img.dim() == 2:
                np_type = NUMPY_TYPE_TO_OPENCV.get(img.__cuda_array_interface__['typestr'])
            ...
            if img.stride()[channel_index] != int(cuda_interface["typestr"][-1]) or \
            ...
            return cv2.cuda.createGpuMatFromCudaMemory(
                (cuda_interface["shape"][width_index], cuda_interface["shape"][height_index]),
                np_type,
                img.__cuda_array_interface__['data'][0],
            )
        ...
        if img.ndim == 3 and (img.shape[channel_index] > 4 or img.shape[channel_index] == 2):
            raise ValueError(
