...
class TestToOpencv:
    ...
    def test_pytorch_channel_first_memory(
        ...
        img_cf_shape_cf_memory = (
            torch.randint(0, 255, size=(3, 10, 20)).to(input_type).to(input_device)
        )
    ...
    def test_pytorch_grayscale(self, input_device, input_type, target_device):
        ...
        img_opencv = as_opencv(img_pytorch, device=target_device)
        np.testing.assert_almost_equal(
            img_opencv if isinstance(img_opencv, np.ndarray) else img_opencv.download(),
    ...
    def test_cupy_first_channel_memory(self, input_type, target_device):
        ...
        # shape - [channels, height, width]
        img_cf_shape_cf_memory = (
            torch.randint(0, 255, size=(3, 10, 20)).to(input_type).to('cuda')
        )
        ...
        img_opencv = as_opencv(img_cl_shape_cf_memory, device=target_device)
        np.testing.assert_almost_equal(
            img_opencv if isinstance(img_opencv, np.ndarray) else img_opencv.download(),
    ...
    def test_cupy_grayscale(self, input_type, target_device):
        ...
        img_opencv = as_opencv(img_cupy, device=target_device)
        np.testing.assert_almost_equal(
            img_opencv if isinstance(img_opencv, np.ndarray) else img_opencv.download(),
            img_cupy.get(),
        )
...
class TestToTorch:
    ...
    def test_opencv(self, input_device, input_type, target_device, output_format):
        # shape - [height, width, channels]
        if input_device == 'cpu':
        ...
        img_torch = as_pytorch(
            img_opencv, output_format=output_format, device=target_device
        )

        np.testing.assert_almost_equal(
            img_opencv if isinstance(img_opencv, np.ndarray) else img_opencv.download(),
            img_torch.cpu().numpy()
            if output_format == 'channels_last'
            else img_torch.permute(1, 2, 0).cpu().numpy(),
        )

    @pytest.mark.parametrize("input_device", ['cuda', 'cpu'])
    @pytest.mark.parametrize("input_type", NUMPY_TYPE)
    @pytest.mark.parametrize("target_device", ['cuda', 'cpu', None])
    def test_opencv_grayscale(self, input_device, input_type, target_device):
        # shape - [height, width]
        if input_device == 'cpu':
    ...
    def test_cupy(self, input_type, target_device, input_format, output_format):
        # shape - [height, width, channels]
        ...
        ) and input_format == 'channels_last':
        ...
        np.testing.assert_almost_equal(img_cupy.get(), img_torch.cpu().numpy())
    ...
    def test_cupy_grayscale(self, input_type, target_device):
        ...
        img_torch = as_pytorch(img_cupy, device=target_device)
...
class TestToCUPY:
    ...
    @pytest.mark.parametrize("input_type", NUMPY_TYPE)
    @pytest.mark.parametrize("output_format", ['channels_first', 'channels_last', None])
    def test_opencv(self, input_device, input_type, output_format):
        # shape - [height, width, channels]
        ...
        np.testing.assert_almost_equal(
            img_opencv if isinstance(img_opencv, np.ndarray) else img_opencv.download(),
            cp.transpose(img_cupy, (1, 2, 0)).get()
    ...
    @pytest.mark.parametrize("input_device", ['cuda', 'cpu'])
    @pytest.mark.parametrize("input_type", NUMPY_TYPE)
    def test_opencv_grayscale(self, input_device, input_type):
        # shape - [height, width]
    ...
    @pytest.mark.parametrize("input_format", ['channels_first', 'channels_last', None])
    @pytest.mark.parametrize("output_format", ['channels_first', 'channels_last', None])
    def test_pytorch(self, input_type, input_device, input_format, output_format):
        # shape - [channels, height, width]
        img_pytorch = (
            torch.randint(0, 255, size=(3, 10, 20)).to(input_type).to(input_device)
        )
        ...
        ) and input_format == 'channels_first':
            image_cupy = cp.transpose(image_cupy, axes=(2, 0, 1))

        np.testing.assert_almost_equal(image_cupy.get(), img_pytorch.cpu().numpy())
    ...
    def test_pytorch_grayscale(
        ...
        # shape - [height, width]
        img_pytorch = (
            torch.randint(0, 255, size=(10, 20)).to(input_type).to(input_device)
        )

        img_cupy = as_cupy(img_pytorch)

        np.testing.assert_almost_equal(img_cupy.get(), img_pytorch.cpu().numpy())
