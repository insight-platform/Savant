...
- ``MAX_ALLOWED_RESOLUTION``: Maximum resolution of the incoming stream. If the resolution is greater than the allowed resolution, the adapter will terminate. You can override the max allowed resolution be setting width and height of frames. The default value is ``3840x2152``.
